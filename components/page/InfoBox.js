		...
		<div className={containerStyle} onClick={(e) => e.stopPropagation()}>
			{post && (
				<>
					...
					<div className={styles.director}>{post.director} · {post.year}</div>
					<div className={styles.summary} onClick={(e) => e.stopPropagation()}>
						...
						<div className={classes(styles.favourite, {[styles.toggled]: favourite}) } onClick={()=>toggleFavourite(post.imdb)}><img src={"/images/plus.svg"} />
						</div>
					</div>
				</>
			)}
